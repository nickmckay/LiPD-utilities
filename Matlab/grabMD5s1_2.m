...
if bagged
    ...
    paleoMeasurementRow=find(~cellfun(@isempty,strfind(md5(:,3),'measurement')) & ~cellfun(@isempty,strfind(md5(:,3),'paleo')));
    chronMeasurementRow=find(~cellfun(@isempty,strfind(md5(:,3),'measurement')) & ~cellfun(@isempty,strfind(md5(:,3),'Chron')));
    chronEnsembleRow=find(~cellfun(@isempty,strfind(md5(:,3),'Ensemble')) & ~cellfun(@isempty,strfind(md5(:,3),'Chron')));
    chronSummaryRow=find(~cellfun(@isempty,strfind(md5(:,3),'summaryTable')) & ~cellfun(@isempty,strfind(md5(:,3),'Chron')));
    paleoEnsembleRow=find(~cellfun(@isempty,strfind(md5(:,3),'Ensemble')) & ~cellfun(@isempty,strfind(md5(:,3),'paleo')));
    paleoSummaryRow=find(~cellfun(@isempty,strfind(md5(:,3),'summaryTable')) & ~cellfun(@isempty,strfind(md5(:,3),'paleo')));
    ...
    for i=1:length(paleoMeasurementRow)
        tose=md5{paleoMeasurementRow(i),3};
        endPiece=strfind(tose,'.measurementTable.csv');
    ...
    for i=1:length(paleoSummaryRow)
        tose=md5{paleoSummaryRow(i),3};
        endPiece=strfind(tose,'.paleoModelTable.csv');
        priorPeriod=max(strfind(tose(1:(endPiece-1)),'.'));
