...
chronRow=find(~cellfun(@isempty,strfind(md5(:,3),'ChronData.csv')));
...
for i=1:length(paleoRow)
    ...
    paleoMD5{i,2}=md5{i,1};
end

chronMD5=cell(length(paleoRow),2);
%assign appropriately
for i=1:length(paleoRow)
    tose=md5{paleoRow(i),3};
    endPiece=strfind(tose,'.ChronData.csv');
    priorPeriod=max(strfind(tose(1:(endPiece-1)),'.'));
    chronMD5{i,1}=tose(priorPeriod+1:endPiece-1);
    chronMD5{i,2}=md5{i,1};
end
