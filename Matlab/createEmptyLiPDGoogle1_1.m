function L=createEmptyLiPDGoogle1_1(dataSetName,nPDT,nCDT,nCol)
...
if nCDT>0
    for i=1:nCol
   E.chronData.chron{i}.chronMeasurementTable.(['dummy' num2str(i)])= E.chronData.chronology.dummy2;
   E.chronData.chron{i}.chronMeasurementTable.(['dummy' num2str(i)]).variableName=['dummy' num2str(i)];
   E.chronData.chron{i}.chronMeasurementTable.(['dummy' num2str(i)]).number=i;

    end
    E.chronData=rmfield(E.chronData,'chronology');
else
...
if nCDT>1
    for i=2:nCDT
    E.chronData.chron{i}= E.chronData.chron{1};
