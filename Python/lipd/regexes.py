...
re_table_name = re.compile(
    r"(paleo|chron)(\d+)(model|measurement)(\d+)(ensemble|summary|distribution)?(\d+)?"
)
re_sci_notation = re.compile(r"(\d+)([.]\d+)(e-\d+)")
re_url = re.compile(
    r"^(http:\/\/www\.|https:\/\/www\.|http:\/\/|https:\/\/)?[a-z0-9]+([\-\.]{1}[a-z0-9]+)*\.[a-z]{2,5}(:[0-9]{1,5})?(\/.*)?$"
)
re_ensemble_collapse = re.compile(r"([\w]*)\-([\w]*)\-ens")

# EXCEL
re_var_w_units = re.compile(
    r"([\w\s\.\-\_\:\/\\\"\'\`\#\+\{\}\[\]\;\*]+)\(?([\w\s\.\-\_\:\/\\\"\'\`\#\+\{\}\[\]\;\*]+)?\)?",
    re.I,
)
# todo write a regex for legacy sheet names
re_sheet = re.compile(
    r"(paleo|chron)[a-zA-Z]*(\d+)(model)?[a-zA-Z]*(\d+)?(measurement|ensemble|summary|distribution)[a-zA-Z]*(\d+)?",
    re.I,
)
re_sheet_w_number = re.compile(
    r"(paleo\d+|chron\d+)(model\d+)?(measurement\d+|ensemble\d+|summary\d+|distribution\d+)?",
    re.I,
)
...
# TIME SERIES
re_misc_fetch = re.compile(
    r"(geo_(\w+)|climateInterpretation_(\w+)|calibration_(\w+)|paleoData_(\w+))"
)
re_pub_fetch = re.compile(
    r"pub1_(citation|year|DOI|author|publisher|title|type|volume|issue|journal|link|pubDataUrl|abstract|pages)"
)

re_pub_valid = re.compile(
    r"pub(\d)_(citation|year|DOI|author|publisher|title|type|volume|issue|journal|link|pubDataUrl|abstract|pages)",
    re.I,
)
...
re_paleo_invalid = re.compile(
    r"paleodata(\w+)|paleodata_(\w+)|measurement(\w+)|measurement_(\w+)"
)
...
# START
re_filter_expr = re.compile(
    r"((\w+_?)\s*(is|in|greater than|equals|equal|less than|<=|==|>=|=|>|<){1}(.*))"
)
