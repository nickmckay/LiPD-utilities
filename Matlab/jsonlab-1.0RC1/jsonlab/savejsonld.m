...
% this is the modification for JSON-LD to write a context file
jsonend=json(2:end);
jsonstart=json(1);
contextFile='context.jsonld';
newstart=[jsonstart '"@context" : "' contextFile '",'];
json=[newstart jsonend];
...
dim=size(item);
%Nick added this to make it work better (no double brackets)
if dim(1)~=1
    item=reshape(item,1,numel(item));
end
dim=size(item);
