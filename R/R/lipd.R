...
lipd_write <- function(j, path, dsn){
  tryCatch({
    ...
    dir_bag_1 <- file.path(dir_tmp, "bag")
    setwd("bag")
    # Need an extra (identical) level for zipping later.
    # dir.create("bag", showWarnings=FALSE)
    # dir_bag_2 <- file.path(dir_tmp,"bag","bag")
    # setwd("bag")
    j <- idx_name_to_num(j)
    ...
    bag.success <- bagit(dir_bag_1, dir_original)
    zipper(dir_original, dir_bag_1, dir_tmp, bag.success, dsn)
    unlink(dir_tmp, recursive=TRUE)
    setwd(dir_original)
  }, error=function(cond){
