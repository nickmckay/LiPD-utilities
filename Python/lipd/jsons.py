...
def read_jsonld():
    ...
    try:
        # Find a jsonld file in cwd. If none, fallback for a json file. If neither found,
        # return empty.
        ...
        if _filename:
            try:
                ...
                logger_jsons.error(
                    "read_jsonld: FileNotFound: {}, {}".format(_filename, fnf)
                )
            except Exception:
                try:
                    _d = demjson.decode_file(
                        _filename, decode_float=float, encoding="latin-1"
                    )
                    logger_jsons.info("Read JSONLD successful: {}".format(_filename))
                except Exception as e:
                    print("Error: unable to read metadata file: {}".format(e))
                    logger_jsons.error(
                        "read_jsonld: Exception: {}, {}".format(_filename, e)
                    )
        else:
            print("Error: metadata file (.jsonld) not found in LiPD archive")
    except Exception:
        print(
            "Error: Unable to find jsonld file in LiPD archive. This may be a corrupt file."
        )
        logger_jsons.error(
            "Error: Unable to find jsonld file in LiPD archive. This may be a corrupt file."
        )
...
def read_json_from_file(filename):
    ...
    try:
        ...
        try:
            d = demjson.decode_file(
                os.path.splitext(filename)[0] + ".json", decode_float=float
            )
        except FileNotFoundError as e:
            # No json or jsonld file. Exit
            print("Error: jsonld file not found: {}".format(filename))
            logger_jsons.debug(
                "read_json_from_file: FileNotFound: {}, {}".format(filename, e)
            )
        except Exception:
    ...
    try:
        for _idx, section in enumerate(sections):
            ...
            if "measurementTable" in section:
                _tmp["measurementTable"] = _idx_table_by_name(
                    section["measurementTable"],
                    "{}{}{}".format(crumbs, _idx, "measurement"),
                )

            # Process the paleo model
            if "model" in section:
                _tmp["model"] = _import_model(
                    section["model"], "{}{}{}".format(crumbs, _idx, "model")
                )

            # Get the table name from the first measurement table,
            # and use that as the index name for this table
            _table_name = "{}{}".format(crumbs, _idx)

            # If we only have generic table names, and one exists already, don't overwrite.
            # Create dynamic name
...
def _import_model(models, crumbs):
    ...
    try:
        for _idx, model in enumerate(models):
            ...
            # Do a direct replacement of chronModelTable columns.
            # No table name, no table work needed.
            if "summaryTable" in model:
                model["summaryTable"] = _idx_table_by_name(
                    model["summaryTable"], "{}{}{}".format(crumbs, _idx, "summary")
                )
            # Do a direct replacement of ensembleTable columns.
            # No table name, no table work needed.
            if "ensembleTable" in model:
                model["ensembleTable"] = _idx_table_by_name(
                    model["ensembleTable"], "{}{}{}".format(crumbs, _idx, "ensemble")
                )
            if "distributionTable" in model:
                model["distributionTable"] = _idx_table_by_name(
...
def _idx_col_by_name(table):
    """
    Iter over columns list. Turn indexed-by-num list into an indexed-by-name dict.
...
def get_csv_from_json(d):
    ...
    :return dict: CSV values. (i.e. { CSVFilename1: { Column1: [Values], Column2: [Values] },
        CSVFilename2: ... }
    """
    ...
    for table, table_content in d[pc].items():
        for column, column_content in table_content["columns"].items():
            try:
                ...
                logger_jsons.debug(
                    "remove_csv_from_json: KeyError: {}, {}".format(pc, e)
                )
...
def write_json_to_file(json_data, filename="metadata"):
    ...
    try:
        ...
        logger_jsons.debug(
            "write_json_to_file: FileNotFound: {}, {}".format(filename, e)
        )
...
def _export_model(models):
    ...
    try:
        for name, model in models.items():
            ...
            if "distributionTable" in model:
                model["distributionTable"] = _idx_table_by_num(
                    model["distributionTable"]
                )
    ...
    try:
        ...
        for _name, _dat in table["columns"].items():
            try:
                ...
                logger_jsons.error(
                    "idx_col_by_num: KeyError: missing number key: {}, {}".format(
                        _name, ke
                    )
                )
            except Exception as e:
